...
export function ImportCSVTable({ fields }: { fields: Field[] }) {
  ...
  const handleSave = async () => {
    ...
    if (!isAtLeastOneFieldMapped(columnMappings)) {
      alert("Veuillez mapper au moins une colonne à un champ")
      return
    }
    ...
    <>
      {csvData.length === 0 && (
        <div className="flex flex-col items-center justify-center gap-2 h-full min-h-[400px]">
          <p className="text-muted-foreground">Téléchargez votre fichier CSV pour importer les transactions</p>
          <div className="flex flex-row gap-5 mt-8">
            <div>
              ...
              <Button type="button" onClick={() => document.getElementById("csv-file")?.click()}>
                {isParsing ? "Analyse..." : <Upload className="mr-2" />} Importer depuis CSV
              </Button>
            </div>
          </div>
        ...
        <div>
          <header className="flex flex-wrap items-center justify-between gap-2 mb-8">
            <h2 className="flex flex-row gap-3 md:gap-5">
              <span className="text-3xl font-bold tracking-tight">Importer {csvData.length} éléments depuis CSV</span>
            </h2>
            <div className="flex gap-2">
              <Button onClick={handleSave} disabled={isSaving}>
                {isSaving ? (
                  <>
                    <Loader2 className="animate-spin" /> Importation en cours...
                  </>
                ) : (
                  <>
                    <Play /> Importer {csvData.length} transactions
                  </>
                )}
              </Button>
            </div>
          ...
          <div className="flex items-center gap-4 mb-4">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span>La première ligne est un en-tête</span>
            </label>
          </div>
          ...
          {csvData.length > MAX_PREVIEW_ROWS && (
            <p className="text-muted-foreground mt-4">et {csvData.length - MAX_PREVIEW_ROWS} autres entrées...</p>
          )}
        </div>
      )}
    </>
