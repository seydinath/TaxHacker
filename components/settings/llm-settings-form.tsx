    ...
    <>
      <form action={saveAction} className="space-y-4">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium">Fournisseurs LLM</label>
          <DndProviderBlocks
          ...
          <small className="text-muted-foreground">
            Glissez les blocs de fournisseurs pour les réorganiser. Le premier a la priorité la plus élevée.
          </small>
        </div>
        <input type="hidden" name="llm_providers" value={providerOrder.join(",")} />

        <FormTextarea
          title="Invite pour le Formulaire d'Analyse de Fichier"
      ...
      <Card className="flex flex-col gap-4 p-4 bg-accent mt-20">
        <CardTitle className="flex flex-row justify-between items-center gap-2">
          ...
          >
            <Edit className="w-4 h-4" /> Modifier les Champs
          </Link>
        </CardTitle>
...
>>>>>>> bcf92d6e9ea2cf4eb86faf3b00924a3577b726c2
      ...
      {provider.apiDoc && (
        <small className="text-muted-foreground">
          Obtenez votre clé API auprès de{" "}
          <a
