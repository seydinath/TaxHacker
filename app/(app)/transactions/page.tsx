...
export default async function TransactionsPage({ searchParams }: { searchParams: Promise<TransactionFilters> }) {
  ...
  return (
    <>
      <header className="flex flex-wrap items-center justify-between gap-2 mb-8">
        ...
        <div className="flex gap-2">
          <ExportTransactionsDialog fields={fields} categories={categories} projects={projects} total={total}>
            <Download /> <span className="hidden md:block">Exporter</span>
          </ExportTransactionsDialog>
          <NewTransactionDialog>
            <Plus /> <span className="hidden md:block">Ajouter une Transaction</span>
          </NewTransactionDialog>
        </div>
      </header>
      ...
      <main>
        <TransactionList transactions={transactions} fields={fields} categories={categories} projects={projects} />
        ...
        {transactions.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-2 h-full min-h-[400px]">
            <p className="text-muted-foreground">
              Vous n'avez pas encore de transactions. Commençons et créez la première!
            </p>
            <div className="flex flex-row gap-5 mt-8">
              <UploadButton>
                <Upload /> Analyser une Nouvelle Facture
              </UploadButton>
              <NewTransactionDialog>
